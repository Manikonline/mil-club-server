...
async function run() {
  try {
    ...
    app.get('/myaddedclasses', async (req, res) => {
      const result = await classesCollection.find({ instructor_email: req.query.email }).toArray()
      res.send(result)
    })

    app.get('/myupdatedclass/:id', async (req, res) => {
      const id = req.params.id;
      const query = { _id: new ObjectId(id) };
    ...
    app.patch('/updatedclass/:id', async (req, res) => {
      ...
      const result = await classesCollection.updateOne(filter, myclassUpdate)
      res.send(result)
    })
    ...
    app.get('/allpayments', async (req, res) => {
      const result = await paymentsCollection.find({ email: req.query.email }).sort({ date: -1 }).toArray()
      res.send(result)
    })
